...
%w[
  ...
  PGP_WORDLIST[i.to_s(16).downcase.rjust(2,'0')] = [evn, odd]
  PGP_EVEN_WORDS[evn.downcase] = hex
  PGP_ODD_WORDS[odd.downcase] = hex
end

def convert_to_pgp_words(hex_string = '')
...
convert_to_pgp_words('E58294F2E9A227486E8B061B31CC528FD7FA3F19').each_slice(4) do |words|
  puts words.join(' ')
end
...
if __FILE__==$PROGRAM_NAME
  ...
  class TestPHPWordlist < Test::Unit::TestCase
    def test_pgp_wordlist_encoding
      assert_equal 'topmost Istanbul Pluto vagabond', convert_to_pgp_words('E58294f2').join(' ')
    end

    def test_pgp_wordlist_decoding
      assert_equal 'e58294f2', convert_from_pgp_words('topmost istanbul  pluto Vagabond')
    end
