...
class String
  def named_scan(regexp)
    names = regexp.names.map(&:to_sym)
    captures = []
    scan(regexp).each do |match|
      captures << Hash[names.zip(match)]
    end
    return captures
  end
end

p 'http://www.desk.com?param1=5&param2=6 www.brainless.org smtp://mail.google.com ftp://ftp.wtf.net '.named_scan(AUTHORITATIVE_URI_RE)

p 'param1=5&param2=6'.named_scan(AUTHORITATIVE_URI_RE)

t = Time.now
1000.times do
  'http://www.desk.com?param1=5&param2=6 www.brainless.org smtp://mail.google.com ftp://ftp.wtf.net '.named_scan(AUTHORITATIVE_URI_RE)
end
